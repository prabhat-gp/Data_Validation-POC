...
function buildSvg(data: TrendPoint[], W: number, H: number): string {
  ...
  data.forEach((r, i) => {
    ...
    out += `<text x="${xs[i].toFixed(1)}" y="${(y - 10).toFixed(1)}" font-size="14" font-weight="800" fill="var(--good)" font-family="ui-monospace,monospace" text-anchor="middle">${fmtNum(r.critical_failed_checks)}</text>`;
  });
  ...
  data.forEach((r, i) => {
    ...
    out += `<text x="${x.toFixed(1)}" y="${(y - 16).toFixed(1)}" font-size="13" font-weight="800" fill="var(--accent)" font-family="ui-monospace,monospace" text-anchor="middle">${label}</text>`;
    out += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="5.5" fill="var(--panel)" stroke="var(--accent)" stroke-width="3"/>`;
  });

  data.forEach((r, i) => {
    out += `<text x="${xs[i].toFixed(1)}" y="${(baseY + 22).toFixed(1)}" font-size="12" font-weight="700" fill="var(--ink)" font-family="ui-monospace,monospace" text-anchor="middle">${r.run_name}</text>`;
  });
